...
def scrape_claude():
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if href.startswith("/en/collections/"):
            full_url = urljoin(CLAUDE_SUPPORT_URL, href)
            collection_urls.add(full_url)
    ...
    # Step 3 — for each article URL, extract article body text
    for article_url in article_urls:
        try:
            response = requests.get(article_url, timeout=10)
            response.raise_for_status()
            ...
            # Step 4 — build chunk with text, source, domain
            chunk = {
                "text": article_text,
                "source": article_url,
                "domain": "claude",
